...
public struct RawMessage
{
    ...
    {
        Source = new(outRawMessage.Source);
            ...
            case "num":
                {
                    ...
                    Bits bits = new(isNegative ? valueStr[3..] : valueStr[2..]);
                    BigInteger x = bits.Parse().LoadUInt(bits.Length);
                    return isNegative ? 0 - x : x;
                }
            case "cell":
                {
                    return Cell.From((string)value.bytes);
                }
            case "list":
            case "tuple":
                {
                    return ParseObject(JObject.FromObject(value));
                }
            default:
                {
